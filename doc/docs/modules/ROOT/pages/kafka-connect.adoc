...
| neo4j.batch.size | Integer | 1000 | The max number of events processed by the Cypher query
| neo4j.batch.timeout.msecs | Integer | 30000 | The execution timeout for the cypher query
| neo4j.batch.parallelize | boolean | true | If enabled messages are processed concurrently in the sink. Non concurrent execution supports in-order processing, e.g. for CDC
