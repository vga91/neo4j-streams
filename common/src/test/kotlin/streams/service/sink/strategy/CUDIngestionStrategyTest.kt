...
import org.neo4j.graph_integration.strategy.cud.CUD
import org.neo4j.graph_integration.strategy.cud.CUDIngestionStrategy
import org.neo4j.graph_integration.strategy.cud.CUDIngestionStrategy.Companion.ID_KEY
...
class CUDIngestionStrategyTest {
    ...
    private fun assertNodeEventsContainsKey(qe: Event, vararg keys: String) = assertTrue {
        qe.events.all {
            val ids = it[ID_KEY] as Map<String, Any>
    ...
    private fun assertRelationshipEventsContainsKey(qe: Event, fromKey: String, toKey: String) = assertTrue {
        qe.events.all {
            ...
            val idsFrom = from[ID_KEY] as Map<String, Any>
            val to = it["to"] as Map<String, Any>
            val idsTo = to[ID_KEY] as Map<String, Any>
    ...
    fun `should create, merge and update nodes`() {
        ...
        val mergeNodeFooBar = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |MERGE (n:Foo:Bar {${key.quote()}: event.$ID_KEY.${key.quote()}})
                |SET n += event.properties
            """.trimMargin(), nodeEvents)
        ...
        val mergeNodeFooBarLabel = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |MERGE (n:Foo:Bar:Label {${key.quote()}: event.$ID_KEY.${key.quote()}})
                |SET n += event.properties
            """.trimMargin(), nodeEvents)
        ...
        val updateNodeFooBar = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |MATCH (n:Foo:Bar {${key.quote()}: event.$ID_KEY.${key.quote()}})
                |SET n += event.properties
            """.trimMargin(), nodeEvents)
        ...
        val updateNodeFooBarLabel = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |MATCH (n:Foo:Bar:Label {${key.quote()}: event.$ID_KEY.${key.quote()}})
                |SET n += event.properties
            """.trimMargin(), nodeEvents)
    ...
    fun `should create, merge, update and delete nodes with garbage data`() {
        ...
        // when
        val cudQueryStrategy = CUDIngestionStrategy<Nothing, CUDNode>()
        val nodeEvents = cudQueryStrategy.mergeNodeEvents(list).events
        val nodeDeleteEvents = cudQueryStrategy.deleteNodeEvents(list).events
        ...
        val mergeNodeFooBar = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |MERGE (n:WellBehaved:`C̸r̵a̵z̵y̵ ̶.̵ ̶ ̴ ̸ ̶ ̶ ̵ ̴L̴a̵b̸e̶l` {`$key`: event.${ID_KEY}.`$key`})
                |SET n += event.properties
            """.trimMargin(), nodeEvents)
        assertEquals(1, mergeNodeFooBar.events.size)
        assertNodeEventsContainsKey(mergeNodeFooBar, key)
        val mergeNodeFooBarLabel = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |MERGE (n:WellBehaved:`C̸r̵a̵z̵y̵ ̶.̵ ̶ ̴ ̸ ̶ ̶ ̵ ̴L̴a̵b̸e̶l`:Label {`$key`: event.${ID_KEY}.`$key`})
                |SET n += event.properties
            """.trimMargin(), nodeEvents)
        assertEquals(2, mergeNodeFooBarLabel.events.size)
        assertNodeEventsContainsKey(mergeNodeFooBarLabel, key)
        val updateNodeFooBar = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |MATCH (n:WellBehaved:`C̸r̵a̵z̵y̵ ̶.̵ ̶ ̴ ̸ ̶ ̶ ̵ ̴L̴a̵b̸e̶l` {`$key`: event.${ID_KEY}.`$key`})
                |SET n += event.properties
            """.trimMargin(), nodeEvents)
        assertEquals(1, updateNodeFooBar.events.size)
        assertNodeEventsContainsKey(updateNodeFooBar, key)
        val updateNodeFooBarLabel = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |MATCH (n:WellBehaved:`C̸r̵a̵z̵y̵ ̶.̵ ̶ ̴ ̸ ̶ ̶ ̵ ̴L̴a̵b̸e̶l`:Label {`$key`: event.${ID_KEY}.`$key`})
                |SET n += event.properties
            """.trimMargin(), nodeEvents)
        assertEquals(1, updateNodeFooBarLabel.events.size)
        assertNodeEventsContainsKey(updateNodeFooBarLabel, key)
    ...
    fun `should create, merge and update relationships only with valid node operations`() {
        ...
        // when
        val cudQueryStrategy = CUDIngestionStrategy<Nothing, CUDRelationship>()
        val nodeEvents = cudQueryStrategy.mergeNodeEvents(list).events
        val nodeDeleteEvents = cudQueryStrategy.deleteNodeEvents(list).events
        ...
        val createRelFooBarLabel = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |CREATE (from:Foo:Bar:Label {key: event.from.${ID_KEY}.key})
                |${IngestionUtils.WITH_EVENT_FROM}
                |MATCH (to:Foo:Bar:Label {key: event.to.${ID_KEY}.key})
    ...
    fun `should delete nodes with internal id reference`() {
        ...
        val deleteNodeFooBar = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |MATCH (n) WHERE id(n) = event.${ID_KEY}._id
                |DELETE n
    ...
    fun `should create, merge and update relationships`() {
        ...
        val createRelFooBar = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |MATCH (from:Foo:Bar {key: event.from.${ID_KEY}.key})
                |${IngestionUtils.WITH_EVENT_FROM}
                |MATCH (to:Foo:Bar {key: event.to.${ID_KEY}.key})
                |CREATE (from)-[r:MY_REL]->(to)
                |SET r = event.properties
            """.trimMargin(), relationshipEvents)
        ...
        val createRelFooBarLabel = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |MATCH (from:Foo:Bar:Label {key: event.from.${ID_KEY}.key})
                |${IngestionUtils.WITH_EVENT_FROM}
                |MATCH (to:Foo:Bar:Label {key: event.to.${ID_KEY}.key})
                |CREATE (from)-[r:MY_REL]->(to)
                |SET r = event.properties
    ...
    fun `should create, merge and update relationships with merge op in 'from' and 'to' node`() {
        ...
        val createRelFooBar = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |MERGE (from:Foo:Bar {key: event.from.${ID_KEY}.key})
                |${IngestionUtils.WITH_EVENT_FROM}
                |MERGE (to:Foo:Bar {key: event.to.${ID_KEY}.key})
                |CREATE (from)-[r:MY_REL]->(to)
                |SET r = event.properties
            """.trimMargin(), relationshipEvents)
        assertEquals(3, createRelFooBar.events.size)
        ...
        val createRelFooBarLabel = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |MERGE (from:Foo:Bar:Label {key: event.from.${ID_KEY}.key})
                |${IngestionUtils.WITH_EVENT_FROM}
                |MERGE (to:Foo:Bar:Label {key: event.to.${ID_KEY}.key})
                |CREATE (from)-[r:MY_REL]->(to)
                |SET r = event.properties
            """.trimMargin(), relationshipEvents)
        assertEquals(2, createRelFooBarLabel.events.size)
    ...
    fun `should create, merge and update relationships with match op in 'from' node and merge or create in 'to' node`() {
        ...
        val matchMergeAndCreateRelFooBar = findEventByQuery("""
                ...
                |MATCH (from:Foo:Bar {key: event.from.${ID_KEY}.key})
                |${IngestionUtils.WITH_EVENT_FROM}
                |CREATE (to:Foo:Bar {key: event.to.${ID_KEY}.key})
                |MERGE (from)-[r:MY_REL]->(to)
                |SET r += event.properties
            """.trimMargin(), relationshipEvents)
    ...
    fun `should create, merge and update relationships with match op in 'to' node and merge or create in 'from' node`() {
        ...
        val matchMergeAndCreateRelFooBar = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |CREATE (from:Foo:Bar {key: event.from.${ID_KEY}.key})
                |${IngestionUtils.WITH_EVENT_FROM}
                |MATCH (to:Foo:Bar {key: event.to.${ID_KEY}.key})
                |MERGE (from)-[r:MY_REL]->(to)
                |SET r += event.properties
    ...
    fun `should delete relationships with internal id reference`() {
        ...
        val deleteRel = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |MATCH (from) WHERE id(from) = event.from.${ID_KEY}._id
                |MATCH (to) WHERE id(to) = event.to.${ID_KEY}._id
                |MATCH (from)-[r:MY_REL]->(to)
                |DELETE r
            """.trimMargin(), relationshipDeleteEvents)
        ...
        val deleteRelFooBarLabel = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |MATCH (from) WHERE id(from) = event.from.${ID_KEY}._id
                |MATCH (to:Foo:Bar:Label {$relKey: event.to.${ID_KEY}.$relKey})
                |MATCH (from)-[r:MY_REL]->(to)
                |DELETE r
            """.trimMargin(), relationshipDeleteEvents)
        assertEquals(5, deleteRelFooBarLabel.events.size)
    ...
    fun `should create, merge, update and delete nodes with compound keys`() {
        ...
        val mergeNodeFooBar = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |MERGE (n:WellBehaved:`C̸r̵a̵z̵y̵ ̶.̵ ̶ ̴ ̸ ̶ ̶ ̵ ̴L̴a̵b̸e̶l` {${firstKey.quote()}: event.${ID_KEY}.${firstKey.quote()}, ${secondKey.quote()}: event.${ID_KEY}.${secondKey.quote()}})
                |SET n += event.properties
            """.trimMargin(), nodeEvents)
        assertEquals(1, mergeNodeFooBar.events.size)
        ...
        val mergeNodeFooBarLabel = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |MERGE (n:WellBehaved:`C̸r̵a̵z̵y̵ ̶.̵ ̶ ̴ ̸ ̶ ̶ ̵ ̴L̴a̵b̸e̶l`:Label {${firstKey.quote()}: event.${ID_KEY}.${firstKey.quote()}, ${secondKey.quote()}: event.${ID_KEY}.${secondKey.quote()}})
                |SET n += event.properties
            """.trimMargin(), nodeEvents)
        assertEquals(2, mergeNodeFooBarLabel.events.size)
        ...
        val updateNodeFooBar = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |MATCH (n:WellBehaved:`C̸r̵a̵z̵y̵ ̶.̵ ̶ ̴ ̸ ̶ ̶ ̵ ̴L̴a̵b̸e̶l` {${firstKey.quote()}: event.${ID_KEY}.${firstKey.quote()}, ${secondKey.quote()}: event.${ID_KEY}.${secondKey.quote()}})
                |SET n += event.properties
            """.trimMargin(), nodeEvents)
        assertEquals(1, updateNodeFooBar.events.size)
        ...
        val updateNodeFooBarLabel = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |MATCH (n:WellBehaved:`C̸r̵a̵z̵y̵ ̶.̵ ̶ ̴ ̸ ̶ ̶ ̵ ̴L̴a̵b̸e̶l`:Label {${firstKey.quote()}: event.${ID_KEY}.${firstKey.quote()}, ${secondKey.quote()}: event.${ID_KEY}.${secondKey.quote()}})
                |SET n += event.properties
            """.trimMargin(), nodeEvents)
        assertEquals(1, updateNodeFooBarLabel.events.size)
    ...
    fun `should create, merge, update and delete nodes without labels`() {
        ...
        val mergeNode = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |MERGE (n {${key.quote()}: event.${ID_KEY}.${key.quote()}})
                |SET n += event.properties
            """.trimMargin(), nodeEvents)
        ...
        val updateNode = findEventByQuery("""
                |${IngestionUtils.UNWIND}
                |MATCH (n {${key.quote()}: event.${ID_KEY}.${key.quote()}})
                |SET n += event.properties
            """.trimMargin(), nodeEvents)
